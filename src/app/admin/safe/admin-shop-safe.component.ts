...
export class AdminShopSafeComponent implements OnInit {
  ...
  list = new ListManager<SafeTransaction>(
    (params) => this.safeService.getShopTransactions(this.shopId, params).pipe(
  ...
  loadSafe() {
    ...
    this.safeService.getShopSafe(this.shopId).subscribe({
  ...
  submitModal() {
    ...
    const req$ = this.modalMode === 'deposit'
      ? this.safeService.adminDeposit(this.shopId, amount, note || undefined)
      : this.safeService.adminWithdraw(this.shopId, amount, note || undefined);
